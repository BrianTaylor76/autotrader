...
export default function StockBrowser({ onSelect }) {
  ...
  const MARKETS = ["All","S&P 500","NASDAQ 100","ETF","Crypto"];
  ...
  const filtered = useMemo(() => {
    ...
    if (fractionalOnly) list = list.filter(s => prices[s.symbol]?.is_fractional);
    if (underHundred) {
      list = list.filter(s => {
        const p = prices[s.symbol]?.price;
        const isHot = prices[s.symbol]?.is_hot;
        return !p || p <= 100 || isHot;
      });
    }
  ...
  const fetchPrices = useCallback(async (stocks) => {
    const toFetch = stocks.filter(s => !getCached(s.symbol)).map(s => s.symbol);
    if (!toFetch.length) {
      const cached = {};
      stocks.forEach(s => { if (getCached(s.symbol)) cached[s.symbol] = getCached(s.symbol); });
      setPrices(prev => ({ ...prev, ...cached }));
      return;
    }
    setLoadingPrices(true);
    const res = await base44.functions.invoke("fetchStockData", { action: "snapshots", symbols: toFetch });
    if (res.data?.snapshots) {
      const snap = res.data.snapshots;
      Object.entries(snap).forEach(([sym, d]) => {
        const enriched = { ...d, is_hot: Math.abs(d.change_pct) > 3 || d.vol_ratio > 2 };
        priceCache[sym] = { ts: Date.now(), data: enriched };
      });
  ...
  useEffect(() => {
    if (pageStocks.length > 0) fetchPrices(pageStocks);
  }, [pageStocks, fetchPrices]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="space-y-3 mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {MARKETS.map(m => (
            <button key={m} onClick={() => setMarket(m)} className={`px-2.5 py-1 rounded text-xs font-medium border transition-colors ${market === m ? "bg-primary/10 text-primary border-primary/30" : "bg-secondary text-muted-foreground border-border hover:text-foreground"}`}>{m}</button>
          ))}
        </div>
        <div className="flex flex-wrap gap-3 text-xs">
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input type="checkbox" checked={underHundred} onChange={e => setUnderHundred(e.target.checked)} className="accent-primary" />
            <span className="text-muted-foreground">Under $100</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input type="checkbox" checked={fractionalOnly} onChange={e => setFractionalOnly(e.target.checked)} className="accent-primary" />
      ...
      <div className="flex-1 overflow-y-auto space-y-0.5 min-h-0">
        {pageStocks.map(stock => {
          ...
          return (
            <button
              key={stock.symbol}
              onClick={() => onSelect({ ...stock, ...d })}
              className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-accent/30 transition-colors text-left group"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {d?.is_fractional && <span className="text-[9px] bg-primary/10 text-primary px-1 rounded font-medium">F</span>}
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-2 border-t border-border mt-2">
          ...
          <span className="text-[10px] text-muted-foreground">{page + 1}/{totalPages} · {filtered.length} stocks</span>
