...
export default function CongressStatsRow({ trades }) {
  const stats = useMemo(() => {
    ...
    return { total: trades.length, buys, sells, topMember, topSymbol };
  }, [trades]);

  if (!stats) return null;

  const cards = [
    { label: "Total Trades", value: stats.total.toLocaleString(), sub: "in dataset" },
    {
      label: "Buys vs Sells",
    ...
    { label: "Most Traded Symbol", value: stats.topSymbol?.[0] || "—", sub: `${stats.topSymbol?.[1] || 0} trades` },
  ];
